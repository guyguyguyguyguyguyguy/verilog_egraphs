...
"""
  uncovered = {a for a in AV if not a & T}
  while uncovered:
    mv = most_freq_var(uncovered) 
    T.add(mv)
    uncovered = {x for x in uncovered if mv not in x}
  return T
\end{python}
...
\begin{python}
def approx_unsatcore(q, A, AV):  # q is a set of, potentially MUS,  variables
  specified = {}
  for vs, a in zip(AV, A):
    q_p = vs.intersection(q - specified)
    if q_p:
      if not is_mus(q_p, a):
        specified.update(q_p)
  return specified
\end{python}
...
\begin{python} 
def ascend_to_boundary(MSA, A, AV):
  specified_vars = approx_unsatcore(V/MSA, A, AV)
...
The code then to get the best approximation of MUS is then:
\begin{python}
def get_mus(variables, assertions, its):
  vars_per_assertion = get_vpa(assertions)
  rel_vars = get_present_vars(assertions)
  msas = search(vars_per_assertion, rel_vars, assertions, its)
  best_msa = min(msas, key=len)

  rel_mus = rel_vars - best_msa
  mus = varaibles - best_msa
